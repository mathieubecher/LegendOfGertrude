...
public class Move : StateMachineBehaviour
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        {
            float angularSpeed = _angularSpeed * (1 + (1 - _controller.rigidbody.velocity.magnitude / _moveSpeed) * 3);
            _controller.transform.rotation = Quaternion.RotateTowards(_controller.transform.rotation, Quaternion.LookRotation(new Vector3(_controller.tilt.x, 0.0f, _controller.tilt.y)),angularSpeed * Time.deltaTime);
